...
//  GitIntroSesiPagi
//
//  Created by Local Administrator on 30/10/21.
//
...
class ConvexHull{
    ...
    func findHull(_ points: [CGPoint], _ p1: CGPoint, _ p2: CGPoint) {
        ...
        convexHull.insert(maxPoint, at: convexHull.firstIndex(of: p1)! + 1) // insert point with max distance from line in the convexHull after p1
    ...
    func area()->CGFloat{
        ...
        for i in convexHull.indices{
            let j = (i+1) % convexHull.count
            area = area + convexHull[i].x * convexHull[j].y
            area = area - convexHull[i].y * convexHull[j].x
        }
        return abs(area/2)
    }
